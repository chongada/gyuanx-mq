...
TEST_CASE("hex encoding/decoding", "[encoding][decoding][hex]") {
    ...
    REQUIRE_FALSE( gyuanxmq::is_hex(odd_hex) );
    REQUIRE_FALSE( gyuanxmq::is_hex("0") );

    REQUIRE( std::all_of(odd_hex.begin(), odd_hex.end(), gyuanxmq::is_hex_digit<char>) );
