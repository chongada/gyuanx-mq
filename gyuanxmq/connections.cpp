...
void GyuanxMQ::rebuild_pollitems() {
    pollitems.clear();
...
void GyuanxMQ::setup_external_socket(zmq::socket_t& socket) {
...
void GyuanxMQ::setup_outgoing_socket(zmq::socket_t& socket, std::string_view remote_pubkey) {
...
ConnectionID GyuanxMQ::connect_sn(std::string_view pubkey, std::chrono::milliseconds keep_alive, std::string_view hint) {
...
ConnectionID GyuanxMQ::connect_remote(const address& remote, ConnectSuccess on_connect, ConnectFailure on_failure,
...
ConnectionID GyuanxMQ::connect_remote(std::string_view remote, ConnectSuccess on_connect, ConnectFailure on_failure,
...
void GyuanxMQ::disconnect(ConnectionID id, std::chrono::milliseconds linger) {
...
GyuanxMQ::proxy_connect_sn(std::string_view remote, std::string_view connect_hint, bool optional, bool incoming_only, bool outgoing_only, std::chrono::milliseconds keep_alive) {
...
std::pair<zmq::socket_t *, std::string> GyuanxMQ::proxy_connect_sn(bt_dict_consumer data) {
...
void GyuanxMQ::proxy_close_connection(size_t index, std::chrono::milliseconds linger) {
...
void GyuanxMQ::proxy_expire_idle_peers() {
...
void GyuanxMQ::proxy_conn_cleanup() {
...
void GyuanxMQ::proxy_connect_remote(bt_dict_consumer data) {
...
void GyuanxMQ::proxy_disconnect(bt_dict_consumer data) {
...
void GyuanxMQ::proxy_disconnect(ConnectionID conn, std::chrono::milliseconds linger) {
